...
tag_dict=dict(filter(lambda x:x[1]>0,tag_dict.items()))
...
song_dict=dict(filter(lambda x : x[1]>5,song_dict.items()))
...
class AE(nn.Module):
    def __init__(self,original_dim):
        ...
        self.fc1=nn.Linear(original_dim,8192)
        self.fc2=nn.BatchNorm1d(8192).cuda()
        self.fc3=nn.Linear(8192,4096).cuda()
        self.fc4=nn.BatchNorm1d(4096).cuda()
        #self.fc3=nn.Linear(4096,2048).cuda()
        #self.fc4=nn.Linear(2048,4096).cuda()
        self.fc5=nn.Linear(4096,8192).cuda()
        self.fc6=nn.BatchNorm1d(8192).cuda()
        self.fc7=nn.Linear(8192,original_dim).cpu()
...
#model=nn.DataParallel(model,devic)
print(model)
for i in range(4):
    print(len(list(model.parameters())[i]))
...
print(x_train_song.shape[0]//batch_size +1)
...
def train(model,x_train_song,epochs):
  ...
  for epoch in range(epochs):
     ...
     for i in range(n_batches_for_epoch):
        ...
        loss_func=weighted_binary_cross_entropy(output,X_batch,weight)
        del output
...
def train(model2,x_train_tag,epochs):
  ...
  for epoch in range(epochs):
     ...
     print("tag epoch time : ", end - start)
...
def rec(pids):
    ...
    for pid in pids:
        ...
        p = p.reshape(1,-1).astype("float32")
        p=torch.from_numpy(p)
        songs_already = test.loc[pid, "songs_id"]
        tags_already = test.loc[pid, "tags_id"]

        with torch.no_grad(): # cand_song : 노래 예측 -> 오토인코더의 결과로 대체
         cand_song = model(p) # p를 넣으면 결과를 뱉어줌.
         cand_song=cand_song.cpu()
         cand_song=cand_song.numpy()
        ...
        t = t.reshape(1,-1).astype("float32")
        t=torch.from_numpy(t).cuda()
        with torch.no_grad():
         cand_tag = model2(t)
         cand_tag=cand_tag.cpu()
         cand_tag=cand_tag.numpy()
